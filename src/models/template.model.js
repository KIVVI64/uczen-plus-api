...
Data.findById = function (id, result) {
    dbConn.query("Select * from templates where id = ? ", id, function (err, res) {             
        if(err) {
            console.log("error: ", err);
            result(err, null);
        }
...
Data.update = function(id, data, result){
  dbConn.query("UPDATE templates SET first_name=?,last_name=?,email=?,phone=?,organization=?,designation=?,salary=? WHERE id = ?", [data.first_name,data.last_name,data.email,data.phone,data.organization,data.designation,data.salary, id], function (err, res) {
        if(err) {
            console.log("error: ", err);
            result(null, err);
        }else{   
